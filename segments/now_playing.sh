...
TMUX_POWERLINE_SEG_NOW_PLAYING_MIN_TERM_COLUMS_DEFAULT="0"
...
# Minimum required columns in the terminal client window. Hide this segment if the terminal is smaller.
export TMUX_POWERLINE_SEG_NOW_PLAYING_MIN_TERM_COLUMS="${TMUX_POWERLINE_SEG_NOW_PLAYING_MIN_TERM_COLUMS_DEFAULT}"
...
run_segment() {
    ...
    if [ "$(tmux display -p -F '#{client_width}')" -lt "$TMUX_POWERLINE_SEG_NOW_PLAYING_MIN_TERM_COLUMS" ]; then
        return 1
    fi
	...
	if [ -n "$np" ]; then
		...
		echo "${TMUX_POWERLINE_SEG_NOW_PLAYING_NOTE_CHAR} ${COLUMS} ${np}"
	fi
	return 0
}
...
__process_settings() {
	...
	if [ -z "$TMUX_POWERLINE_SEG_NOW_PLAYING_MIN_TERM_COLUMS" ]; then
		export TMUX_POWERLINE_SEG_NOW_PLAYING_MIN_TERM_COLUMS="${TMUX_POWERLINE_SEG_NOW_PLAYING_MIN_TERM_COLUMS_DEFAULT}"
	fi
